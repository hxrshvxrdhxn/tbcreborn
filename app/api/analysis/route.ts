...
async function isRateLimited(ip: string): Promise<boolean> {
  if (upstashRatelimit) {
    try {
      const { success } = await upstashRatelimit.limit(`analysis_${ip}`);
      return !success;
    } catch (e) {
...
const schema = z.object({
  ...
  request: z.string().max(5000).optional(),
...
export async function POST(req: NextRequest) {
  try {
    ...
    await transporter.sendMail({
      ...
      subject: `Free Analysis Request: ${esc(data.company)}`,
      html: `
        <h2>New Free Analysis Request</h2>
        ...
        <p style="white-space:pre-wrap">${esc(data.request || "None")}</p>
      `,
    });
    ...
    if (sheetsWebhook) {
      // Create a specific payload for the free analysis, or generic data
      const sheetsData = {
        type: "FREE_ANALYSIS",
        name: data.name,
    ...
    console.error("Free Analysis form error:", err);
